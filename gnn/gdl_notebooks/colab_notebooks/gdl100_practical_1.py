# -*- coding: utf-8 -*-
"""GDL100 - Practical 1.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/13m6BDOnouhyaw-CbjyBYwVDAjie8NqUx
...
Mathematically we can define simple graph as an ordered tuple ${G} = (V, E)$ where $V$ is a set of nodes (or vertices) and $E \subseteq (V \times V$) is a 2-tuple set of the edges in the graph. Hence if $u$ and $v$ are nodes in ${G}$, their relation is recorded with edge as $(u,v) \in E$ if the edge is directed from $u$ to $v$. The neighbours of a node $u$ is the set of nodes which share an edge with $u$, denoted ${N}(u) = \{v | (u, v) \in E\}$.
...
In lectures 3-4 we were introduced to a general blue-print for creating GNN layers. We can build GNN layers $\mathbf{F(X,A)}$ on graphs via the shared application of local permutation invariant $\phi(\mathbf{x_i}, \mathbf{X}_{{N}_i})$. The various implementations of $\phi$ is the subject of much research, for our purpose we will start by considering GNN layers with constant values for the convolution coefficients.
...
### 💻**Task 1.1**  Define $c_{i,j}$, $\oplus_{j \in {N}_i}$ and $\phi$ _mathematically_ so that the behaviour of the generic, convolutional GNN layer is the same as a GCN. (0.5 Marks)
...
The GCNLayer fucoses on the immediate neighbors (by left multiplying with the adjacency matrix), while the SimpleMLP takes into account all the nodes in the graph.
...
Make it a fully connected graph ?
This way the adjacency matrix will be all ones, with no importance of local neighberhoods, and we should reduce back to a simple MLP
...
one_graph.edge_index.shape

torch.cat((one_graph.edge_index, one_graph.edge_index), dim=1).shape
